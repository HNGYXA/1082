...
/***
	功能：发送消息到队列(中断亦可用此函数，timeout需设置为0)
	参数1：mq_id：队列句柄。
...
/***
	功能：从队列接收消息(中断亦可用此函数，timeout需设置为0)
	参数1：mq_id：队列句柄。
...
/***
功能:重置队列状态：将队列的状态恢复到初始创建时的状态，包括：队列计数清零（消息数量为 0）。重置队列锁和等待任务列表。
	参数1：mq_id：要重置的消息队列 ID（由 osMessageQueueNew() 返回）。
	返回值：
		osOK：成功重置队列。
		osErrorParameter：无效的队列 ID。
		osErrorResource：队列当前正在被使用，无法重置。
